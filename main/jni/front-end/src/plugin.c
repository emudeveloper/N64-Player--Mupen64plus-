...
static m64p_error PluginLoadTry(const char *filepath, int MapIndex)
{
    ...
    if (PluginGetVersion == NULL)
    {
        if (g_Verbose)
        {
            ...
            __android_log_print(ANDROID_LOG_ERROR, "front-end", "Error: library '%s' is not a Mupen64Plus library.\n", filepath);
        }
    ...
    if (PluginStartup == NULL)
    {
        ...
        __android_log_print(ANDROID_LOG_ERROR, "front-end", "Error: library '%s' broken.  No PluginStartup() function found.\n", filepath);
        osal_dynlib_close(handle);
        return M64ERR_INCOMPATIBLE;
    }
    ...
    if (rval != M64ERR_SUCCESS)
    {
        ...
        __android_log_print(ANDROID_LOG_ERROR, "front-end", "Error: %s plugin library '%s' failed to start.\n", g_PluginMap[MapIndex].name, filepath);
        osal_dynlib_close(handle);
        return rval;
    }
...
m64p_error PluginSearchLoad(m64p_handle ConfigUI)
{
    ...
    if (g_PluginDir != NULL)
    {
        ...
        if (lib_filelist == NULL)
        {
            ...
            __android_log_print(ANDROID_LOG_ERROR, "front-end", "Error: No plugins found in --plugindir path: %s\n", g_PluginDir);
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (cmdline_path != NULL)
        {
            ...
            if (!use_dummy && g_PluginMap[i].handle == NULL)
            {
                ...
                __android_log_print(ANDROID_LOG_ERROR, "front-end", "Error: Specified %s plugin not found: %s\n", g_PluginMap[i].name, cmdline_path);
        ...
        if (g_PluginMap[i].handle == NULL)
        {
            ...
            __android_log_print(ANDROID_LOG_VERBOSE, "front-end", "UI-console: using %s plugin: <dummy>\n", g_PluginMap[i].name);
        }
        else
        {
            printf("UI-console: using %s plugin: '%s' v%i.%i.%i\n", g_PluginMap[i].name,
                   g_PluginMap[i].libname, VERSION_PRINTF_SPLIT(g_PluginMap[i].libversion));
            __android_log_print(ANDROID_LOG_VERBOSE, "front-end", "UI-console: using %s plugin: '%s' v%i.%i.%i\n", g_PluginMap[i].name,
                    g_PluginMap[i].libname, VERSION_PRINTF_SPLIT(g_PluginMap[i].libversion));
            if (g_Verbose)
            {
                printf("UI-console: %s plugin library: %s\n", g_PluginMap[i].name, g_PluginMap[i].filename);
                __android_log_print(ANDROID_LOG_VERBOSE, "front-end", "UI-console: %s plugin library: %s\n", g_PluginMap[i].name, g_PluginMap[i].filename);
            }
        }
    }
