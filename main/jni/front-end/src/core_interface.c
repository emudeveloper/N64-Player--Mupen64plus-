...
#define printf(...) __android_log_print(ANDROID_LOG_VERBOSE, "core_interface", __VA_ARGS__)
...
m64p_error AttachCoreLib(const char *CoreLibFilepath)
{
    ...
    if (rval != M64ERR_SUCCESS || CoreHandle == NULL)
    {
        ...
        __android_log_print(ANDROID_LOG_ERROR, "front-end", "AttachCoreLib() Error: failed to find Mupen64Plus Core library\n");
        CoreHandle = NULL;
    ...
    if (CoreVersionFunc == NULL)
    {
        ...
        __android_log_print(ANDROID_LOG_ERROR, "front-end", "AttachCoreLib() Error: Shared library '%s' invalid; no PluginGetVersion() function found.\n", CoreLibFilepath);
