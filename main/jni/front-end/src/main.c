...
void Java_com_emudev_n64_SDLActivity_pauseEmulator(
                                    JNIEnv* env, jclass cls)
{
    (*CoreDoCommand) ( M64CMD_PAUSE, 0, NULL );
}

void Java_com_emudev_n64_SDLActivity_resumeEmulator(
                                    JNIEnv* env, jclass cls)
{
    (*CoreDoCommand) ( M64CMD_RESUME, 0, NULL );
}

void Java_com_emudev_n64_SDLActivity_stopEmulator(
                                    JNIEnv* env, jclass cls)
{
    (*CoreDoCommand) ( M64CMD_STOP, 0, NULL );
}

void Java_com_emudev_n64_SDLActivity_stateSetSlotEmulator(
                                    JNIEnv* env, jclass cls, jint slotID )
{
    (*CoreDoCommand) ( M64CMD_STATE_SET_SLOT, (int) slotID, NULL );
}
void Java_com_emudev_n64_SDLActivity_stateSaveEmulator(
                                    JNIEnv* env, jclass cls)
{
    (*CoreDoCommand) ( M64CMD_STATE_SAVE, 1, NULL );
}
void Java_com_emudev_n64_SDLActivity_stateLoadEmulator(
                                    JNIEnv* env, jclass cls)
{
    (*CoreDoCommand) ( M64CMD_STATE_LOAD, 0, NULL );
}

void Java_com_emudev_n64_SDLActivity_fileSaveEmulator(
...
void Java_com_emudev_n64_SDLActivity_fileLoadEmulator(
                                    JNIEnv* env, jclass cls, jstring filename )
{
    const char *nativeString = (*env)->GetStringUTFChars( env, filename, 0 );
...
jint Java_com_emudev_n64_SDLActivity_stateEmulator(
                                    JNIEnv* env, jclass cls)
{
    int state = 0;
...
static m64p_error OpenConfigurationHandles(void)
{
    ...
    (*ConfigSetDefaultString)(l_ConfigUI, "VideoPlugin", "libvideo-rice" OSAL_DLL_EXTENSION, "Filename of video plugin");
...
int main(int argc, char *argv[])
{
    int i;
//    char appHomePath[60];
//    strcpy( appHomePath, "/mnt/sdcard/app-data/paulscode.android.mupen64plus/");
//    if( chdir( appHomePath ) != 0 )
    ...
    // Hack to allow configuration file to be in home directory
