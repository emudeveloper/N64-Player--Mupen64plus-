...
#define printf(...) __android_log_print(ANDROID_LOG_VERBOSE, "input-sdl, plugin.c", __VA_ARGS__)
...
unsigned char virtualGamePadButtonState[14];
signed char virtualGamePadAxisState[2];
void Java_com_emudev_n64_SDLActivity_updateVirtualGamePadStates(
                                    ...
                                    jbooleanArray mp64pButtons, jint mp64pXAxis, jint mp64pYAxis )
{
    ...
    for( b = 0; b < 14; b++ )
    {
        virtualGamePadButtonState[b] = elements[b];
    }
    ...
    virtualGamePadAxisState[0] = (signed char) ((int) mp64pXAxis);
    virtualGamePadAxisState[1] = (signed char) ((int) mp64pYAxis);
}
////
...
static void doVirtualGamePad()
{
    ...
    for( b = 0; b < 14; b++ )
    {
        if( virtualGamePadButtonState[b] )
            controller[0].buttons.Value |= button_bits[b];
    }
    // from the N64 func ref: The 3D Stick data is of type signed char and in
    // the range between 80 and -80. (32768 / 409 = ~80.1)
    if( virtualGamePadAxisState[0] || virtualGamePadAxisState[1] )
    {
        // only report the stick position if it isn't back at the center
        controller[0].buttons.X_AXIS = virtualGamePadAxisState[0];
        controller[0].buttons.Y_AXIS = virtualGamePadAxisState[1];
    }
}
