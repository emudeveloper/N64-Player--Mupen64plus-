...
#ifdef GLES_2
    m_convTexture = malloc(m_dwCreatedTextureWidth * m_dwCreatedTextureHeight * 32);
#endif
...
//    #define GL_BGRA GL_BGRA_IMG
    int totalPixels = m_dwCreatedTextureWidth * m_dwCreatedTextureHeight;
    int x;
    unsigned char *newPtr = (unsigned char *) m_convTexture;
    unsigned char *oldPtr = (unsigned char *) m_pTexture;
    for( x = 0; x < totalPixels; x++ )
    {
        memcpy( newPtr, oldPtr + 2, 8 );
        memcpy( newPtr + 1, oldPtr + 1, 8 );
        memcpy( newPtr + 2, oldPtr, 8 );
        memcpy( newPtr + 3, oldPtr + 3, 8 );
        newPtr += 4;
        oldPtr += 4;
    }
    glTexImage2D( GL_TEXTURE_2D, 0, m_glFmt, m_dwCreatedTextureWidth, m_dwCreatedTextureHeight,
                  0, GL_RGBA, GL_UNSIGNED_BYTE, m_convTexture );
#else
