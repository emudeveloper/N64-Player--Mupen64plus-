...
static int get_xdg_dir(char *destpath, const char *envvar, const char *subdir)
{
	...
	if( envvar == NULL || strlen(envvar) < 1 )
		printf( "    Attempted to get_xdg_dir for undefined envvar" );
	else
        	printf( "    envpath for '%s' was null or length < 1", envvar );
        return 1;
    }
    ...
    if (stat(envpath, &fileinfo) != 0 || !S_ISDIR(fileinfo.st_mode))
    {
	    ...
	    printf( "    Path '%s' is not a directory", envpath );
        else
            printf( "    Path '%s' is not valid", envpath );
        return 2;
    }
